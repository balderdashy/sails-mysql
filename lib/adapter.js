...
module.exports = (function() {
  ...
  // Keep a cache of all the schema keys
  var schema = {};
  ...
  var adapter = {
    ...
    registerCollection: function(collection, cb) {
      ...
      // Store Definition
      schema[key] = _.clone(def).definition;
    ...
    drop: function(collectionName, relations, cb) {
      ...
      spawnConnection(function __DROP__(connection, cb) {
        ...
        function dropTable(item, next) {
          ...
          // Build query
          var query = 'DROP TABLE ' + mysql.escapeId(item);

          // Run query
          connection.query(query, function __DROP__(err, result) {
            if (err) {
              if (err.code !== 'ER_BAD_TABLE_ERROR') return next(err);
              result = null;
            }
        ...
        async.eachSeries(relations, dropTable, function(err) {
          if(err) return cb(err);
          dropTable(dbs[collectionName].identity, cb);
        });
    ...
    find: function(collectionName, options, cb) {
      spawnConnection(function(connection, cb) {
        ...
        // Build find query
        var query = sql.selectQuery(dbs[collectionName].identity, options, schema);
