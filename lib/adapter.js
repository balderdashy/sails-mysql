...
var adapter = {
    ...
    registerCollection: function(collection, cb) {
        ...
        var def.definition['primaryKeyName'] = 'id';

        // Register Primary key fieldname
        for (var item in def.definition) {
            // Check if the model definition has primary key
            if (_.has(def.definition[item], 'primaryKey') && def.definition[item].primaryKey) {
                // Check if the model definition has a columnName define
                if ( _.has(def.definition[item], 'columnName') ){
                    // Set the primary key to the columnName value
                    def.definition['primaryKeyName'] = def.definition.columnName
                } else {
                    // Default to the item name
                    def.definition['primaryKeyName'] = item;
                }; 
                break;
            }
        }
    ...
    createEach: function(collectionName, valuesList, cb) {
        spawnConnection(function(connection, cb) {
            ...
            async.eachSeries(valuesList, function(data, cb) {
                ...
                var query = "SELECT * FROM " + dbs[collectionName].identity + " WHERE " + dbs[collectionName].definition.primaryKeyName + " IN (" + records + ");";
    ...
    update: function(collectionName, options, values, cb) {
        spawnConnection(function(connection, cb) {
            ...
            var query = 'SELECT ' + dbs[collectionName].definition.primaryKeyName + ' FROM ' + tableName + ' ' + criteria;
            ...
            connection.query(query, function(err, results) {
                ...
                connection.query(query, function(err, result) {
                    ...
                    if (ids.length === 1) {
                        ...
                        criteria.where[dbs[collectionName].definition.primaryKeyName] = ids[0];
                    } else {
                        criteria = {
                            where: {}
                        };
