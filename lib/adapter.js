  ...
  var adapter = {
    ...
    define: function(connectionName, collectionName, definition, cb, noop, connection) {
      ...
      function __DEFINE__(connection, cb) {
        ...
        if(connectionObject.config.engine) {
          query += ' ENGINE=' + connectionObject.config.engine;
        }

        if(connectionObject.config.rowFormat) {
          query += ' ROW_FORMAT=' + connectionObject.config.rowFormat;
        }
            ...
            async.auto({
              ...
              processChildren: ['buildChildBuffers', function(resultsSoFar, next) {
                ...
                async.each(_query.query, function(q, next) {
                  ...
                  // use subqueries and combine results by UNION
                  // if limit or skip options are set on populate
                  // otherwise we could use IN comparision operate
                  // which performs much better
                  if (
                    q.instructions !== undefined &&
                    q.instructions.criteria !== undefined &&
                    (
                      q.instructions.criteria.limit !== undefined ||
                      q.instructions.criteria.skip !== undefined
                    )
                  ) {
                    ...
                    qs = q.qs.replace('= ^?^', 'IN (' + parentRecords.map(function(parent) {
                      var parentPk = parent[pk];
                      return _.isNumber(parentPk) ? parentPk : '"' + parentPk + '"';
                    }).join(', ') + ')');

                    // remove clinches around query
                    qs = qs.replace(/^\s*\(/, '').replace(/\s*\)$/, '');
                  }
    ...
    destroy: function(connectionName, collectionName, options, cb, noop, connection) {
      ...
      function __DESTROY__(connection, cb) {
        ...
        async.auto({
          ...
          destroyRecords: ['findRecords', function(resultsSoFar, next) {
