...
module.exports = (function() {
  ...
  /**
   * Clean the schema of any collections that don't belong to this adapter.
   * Transfer any keys that are different from their tableName.
   *
   * @param {Object} schema
   */
  ...
  function cleanseSchema (schema, connectionName) {
    ...
    _.each(_.keys(schema), function(key) {
      var collection = schema[key];

      if(!_.includes(collection.connection, connectionName)) {
        delete _schema[key];
        return;
      }

      if(collection.tableName != key) {
        delete _schema[key];
        _schema[collection.tableName] = collection;
      }
