  ...
  var adapter = {
    ...
    create: function(connectionName, collectionName, data, cb, connection) {
      ...
      function __CREATE__(connection, cb) {
        ...
        var schema = {};
        Object.keys(collection.waterline.schema).forEach(function(coll) {
          if (collection.waterline.schema[coll].connection.indexOf(connectionName) !== -1) {
            schema[collection.waterline.schema[coll].identity] = collection.waterline.schema[coll]
          };
        });
