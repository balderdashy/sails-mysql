...
module.exports.configure = function ( connections ) {
  ...
  return function registerConnection (connection, collections, cb) {
    ...
    // Validate arguments
    if(!connection.identity) return cb__registerConnection(Errors.IdentityMissing);
    if(connections[connection.identity]) return cb__registerConnection(Errors.IdentityDuplicate);
