...
module.exports = function spawnConnection (queryType, connectionObject, fn, cb__spawnConnection) {
  //
  // TODO:
  //
  ...
  // logic for the query.
  if (connectionObject.connection.poolCluster) {
    connectionObject.connection.poolCluster.getConnection(queryType + '*', function (err, conn) {
      if (err && err.code === 'POOL_NOEXIST') {
        err = new Error(queryType + ' pool does not exist.');
    ...
    return;
  } else if (connectionObject.connection.pool) {
