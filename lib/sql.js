...
var sql = {
  ...
  serializeOptions: function(collectionName, options, tableDefs) {
    ...
    if ((options.join && options.join.length > 0) || (options.joins && options.joins.length > 0)) {

      var joins = options.joins || option.join;

      joins.forEach(function(join) {
        var tableName = join.toLowerCase(),
            foreignKey, collection, parentKey;

        /**
         * Determine which side of the join has the foreign key
         */

        // Check collectionName
        Object.keys(tableDefs[collectionName]).forEach(function(key) {
          var cache = tableDefs[collectionName][key];

          if(cache.hasOwnProperty('foreignKey') && key === join.toLowerCase() + '_id') {
            foreignKey = mysql.escapeId(collectionName) + '.' + mysql.escapeId(key);
            parentKey = mysql.escapeId(join.toLowerCase()) + '.' + mysql.escapeId('id');
          }
        });

        // If not there check join table
        if(!foreignKey) {
          var _join = join.toLowerCase();

          Object.keys(tableDefs[_join]).forEach(function(key) {
            var cache = tableDefs[_join][key];

            if(cache.hasOwnProperty('foreignKey') && key === collectionName.toLowerCase() + '_id') {
              foreignKey = mysql.escapeId(_join) + '.' + mysql.escapeId(key);
              parentKey = mysql.escapeId(collectionName.toLowerCase()) + '.' + mysql.escapeId('id');
            }
          });
        }
        ...
        // Build Join Clause
        queryPart += 'INNER JOIN ' + tableName + ' AS ' + tableName + ' ON ' +
    ...
    if (options.limit) {
      ...
      // Only apply limit if no joins are specified
      if((options.joins && options.joins.length > 0) || (options.join && options.join.length > 0)) {
